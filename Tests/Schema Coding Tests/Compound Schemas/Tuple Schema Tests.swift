...
struct TupleSchemaTests {
  ...
  private func testSchemaEncoding() throws {
    #expect(
      SchemaProvider.schema(representing: (String, Int).self).schemaJSON == """
        {
          "prefixItems": [
            {
              "type": "string"
  ...
  private func testValueEncoding() throws {
    let schema1 = SchemaProvider.schema(representing: (String, Int).self)
    ...
    let schema2 = SchemaProvider.schema(representing: (String, Int, Double).self)
  ...
  private func testValueDecoding() throws {
    let stringIntSchema = SchemaProvider.schema(representing: (String, Int).self)
    ...
    let stringIntDoubleSchema = SchemaProvider.schema(representing: (String, Int, Double).self)

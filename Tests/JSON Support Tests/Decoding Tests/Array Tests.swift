...
struct ArrayTests {
  ...
  func emptyArrayTest() async throws {
    do {
      ...
      let result = try stream.decodeArrayComponent(state: &state)
  ...
  func simpleArrayTest() async throws {
    ...
    do {
      ...
      while case .decoded(.elementStart) = try stream.decodeArrayComponent(state: &state) {
  ...
  func arrayOfStringsTest() async throws {
    do {
      ...
      while try stream.decodeArrayComponent(state: &arrayState).isElementStart {
        var state = JSON.StringFragmentDecodingState()
  ...
  func mixedTypeArrayTest() async throws {
    ...
    do {
      ...
      var arrayState = JSON.ArrayDecodingState()
      var result = try stream.decodeArrayComponent(state: &arrayState)
      ...
      result = try stream.decodeArrayComponent(state: &arrayState)
      #expect(result.isElementStart)
  ...
  func incrementalArrayParsingTest() async throws {
    ...
    do {
      ...
      var result = try stream.decodeArrayComponent(state: &state)
      #expect(result.isIncomplete)

      stream.push("12")
      result = try stream.decodeArrayComponent(state: &state)
      #expect(result.isElementStart)

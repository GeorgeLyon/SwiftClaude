...
struct ObjectTests {
  ...
  func emptyObjectTest() async throws {
    do {
      ...
      let result = try stream.decodeObjectComponent(state: &state)
  ...
  func simpleObjectTest() async throws {
    ...
    do {
      ...
      var state = JSON.ObjectDecodingState()
      var result = try stream.decodeObjectComponent(state: &state)

      while case .decoded(.propertyValueStart(let name)) = result {
        // Decode value
        var valueState = JSON.StringFragmentDecodingState()
        ...
        result = try stream.decodeObjectComponent(state: &state)
      }
  ...
  func nestedObjectTest() async throws {
    do {
      ...
      while case .decoded(.propertyValueStart(let name)) = result {
        ...
        var nestedResult = try stream.decodeObjectComponent(state: &nestedState)
        while case .decoded(.propertyValueStart(let nestedName)) = nestedResult {
          if nestedName == "age" {
            ...
            var stringState = JSON.StringFragmentDecodingState()
          ...
          nestedResult = try stream.decodeObjectComponent(state: &nestedState)
        }
  ...
  func incrementalObjectParsingTest() async throws {
    ...
    do {
      ...
      let incomplete = try stream.decodeNumber().isIncomplete
      #expect(incomplete)
  ...
  func deeplyNestedObjectTest() async throws {
    do {
      ...
      for level in ["a", "b", "c", "d", "e"] {
        ...
        states.append(state)
        let result = try stream.decodeObjectComponent(state: &states[states.count - 1])
      ...
      for i in (0..<5).reversed() {
        let result = try stream.decodeObjectComponent(state: &states[i])
        #expect(result.isComplete)
      }
    }
  }
  ...
  func objectWithArrayValuesTest() async throws {
    do {
      ...
      while case .decoded(.elementStart) = try stream.decodeArrayComponent(state: &arrayState) {
        let number = try stream.decodeNumber().getValue()
        let intValue = try number.decode(as: Int.self)
      ...
      while case .decoded(.elementStart) = try stream.decodeArrayComponent(state: &arrayState2) {
        var stringState = JSON.StringFragmentDecodingState()
        var fragments: [String] = []
        _ = try stream.decodeStringFragments(state: &stringState) { fragment in
          fragments.append(String(fragment))

extension JSON {
  ...
  public struct StringDecodingState: Sendable {

    public init() {

    }

    fileprivate var fragments: [Substring] = []
    fileprivate var fragmentsState = StringFragmentDecodingState()

  }

  public struct StringFragmentDecodingState: Sendable {
...
extension JSON.DecodingStream {
  ...
  ) throws -> JSON.DecodingResult<Substring> {
    try readString(state: &state).decodingResult()
  }

  public mutating func decodeStringFragments(
    state: inout JSON.StringFragmentDecodingState,
  ...
  mutating func readString() -> ReadResult<Substring> {
    ...
    let result = readString(state: &state)
    switch result {
    case .incomplete:
      restore(checkpoint)
      return .incomplete
  ...
  mutating func readString(
    ...
    let result = readStringFragments(state: &state.fragmentsState) { fragment in
    ...
    case .notMatched(let error):
      return .notMatched(error)
    case .matched(.end):
      if state.fragments.count == 1 {
        /// Fast path for a single fragment
        return .matched(state.fragments[0])
      } else {
        /// Join all fragments
        return .matched(Substring(state.fragments.joined()))
      }
    }
  }
  ...
  /// Unlike other `read` methods, this method advances the read cursor even if `incomplete` is returned.

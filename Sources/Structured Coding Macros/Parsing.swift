    ...
    guard name.identifier != nil else {
      context.diagnose(
        DiagnosticError(
          node: name,
          severity: .error,
          message: "@StructuredAction requires a function with an identifier name"
        )
      )
      return nil
    }
      ...
      case .keyword(.mutating), .keyword(.borrowing), .keyword(.consuming):
        context.diagnose(
          DiagnosticError(
            node: modifier,
            severity: .error,
            message: "@StructuredAction does not support `\(modifier.name.trimmed)` methods"
          )
        )
        return nil
      default:
    ...
    case .some:
      context.diagnose(
        DiagnosticError(
          ...
          severity: .error,
          message: "@StructuredAction does not support `reasync`"
        )
      )
      return nil
    }
      ...
      if let specifier = parameter.type.as(AttributedTypeSyntax.self)?.specifiers.first {
        context.diagnose(
          DiagnosticError(
            node: parameter,
            severity: .error,
            message: "@StructuredAction does not support `\(specifier.trimmed)` parameters"
          )
        )
        return nil
      }

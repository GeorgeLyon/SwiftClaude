extension Arena {
  ...
  final class BitwiseCopyableSlab: Arena.Slab {
    ...
    func reset(_ buffer: UnsafeMutableRawBufferPointer) {
      guard isMutable == false else { fatalError() }
      forEachComponent(in: buffer) { component, pointer in
...
extension Arena.BitwiseCopyableSlab {
  ...
  fileprivate protocol ComponentType {
    static func initialValue() -> Self
  }

  fileprivate struct Component {

    let typeExistential: any (ComponentType & BitwiseCopyable).Type

    func increment(_ cursor: inout UnsafeMutableRawPointer) -> UnsafeMutableRawPointer {
      func increment<T: ComponentType & BitwiseCopyable>(_ type: T.Type) -> UnsafeMutableRawPointer
      {
    ...
    func initialize(_ pointer: UnsafeMutableRawPointer) {
      func initialize<T: ComponentType & BitwiseCopyable>(_ type: T.Type) {
        pointer
    ...
    func reset(_ pointer: UnsafeMutableRawPointer) {
      func reset<T: ComponentType & BitwiseCopyable>(_ type: T.Type) {
        pointer
    ...
    func deinitialize(_ pointer: UnsafeMutableRawPointer) {
      func deinitialize<T: ComponentType & BitwiseCopyable>(_ type: T.Type) {
        pointer
          .assumingMemoryBound(to: T.self)
...
extension Optional: Arena.BitwiseCopyableSlab.ComponentType {

...
extension SchemaCoding.Support.ObjectSchema {
    ...
    Properties == (repeat each Property)
  {
    ...
    return
      (repeat SchemaCoding.Support.NamedProperty(
        name: (each propertyNames).name,
...
extension SchemaCoding.Support.ObjectSchema {
    ...
    PropertyValues == (repeat (each Property).Schema.Value)
  {
    ...
    encoder.stream.encodeObject { objectEncoder in
      var encoder = SchemaCoding.Support.ObjectPropertiesEncoder(objectEncoder: objectEncoder)
      encodePropertyValues((repeat each propertyValues), to: &encoder)
  ...
  where
    ...
    PropertyValues == (repeat (each Property).Schema.Value)
  {
    let namedProperties = namedProperties()
    for (property, value) in repeat (each namedProperties, each values) {
      let propertyValue = type(of: property.property).propertyValue(from: value)
      if let propertyValue,
        let propertySchema = property.property.propertySchema
      {
        encoder.objectEncoder.encodeProperty(name: property.name) { stream in
          stream.encode(propertyValue, using: propertySchema)
        }
      } else {
        /// Omitted properties should not have values
...
extension SchemaCoding.Support.ObjectSchema {
  ...
  public func decodeValue(
    ...
    while true {
      while true {
        if let propertyDecoder = state.activeDecoder {
          switch try propertyDecoder.decodeValue(from: &decoder).kind {
          case .incomplete:
            return .incomplete
          case .decoded:
            state.activeDecoder = nil
          }
        }
        ...
        case .decoded(.propertyValueStart(let name)):
          ...
          state.activeDecoder = propertyDecoder
        case .decoded(.end):
  ...
  {
    let propertyNames = Self.propertyNames()
    return SchemaCoding.Support.ObjectPropertiesDecoder(
      propertyNames: (repeat each propertyNames),
...
extension SchemaCoding.Support {
    ...
    Schema.PropertyValues == (repeat (each Property).Schema.Value)
  {
    ...
    init(
      ...
      self.propertyDecoders = (repeat (each properties).beginDecoding(from: decoder))
  ...
  >: ObjectPropertiesDecoderProtocol {
    ...
    init(
      ...
      for component in repeat each otherComponents {
        propertyDecodersByName.merge(component.propertyDecodersByName)
      }
      self.propertyDecodersByName = propertyDecodersByName
    }
  ...
  struct PropertyDecodersByName {
    ...
    init<each Property: ObjectProperty>(
      ...
      self.init()
      for (name, decoder) in repeat (each propertyNames, each decoders) {
        self[Substring(name.name.stringValue)] = decoder
      }
    }
...
extension SchemaCoding.Support {
    ...
    Value.Properties == (repeat each ValueProperty)
  {
    ...
    static func propertyNames() -> PropertyNames {
      (repeat (each Value.propertyNames()).map())
    }
    ...
    static func value(from propertyValues: PropertyValues) throws -> Value {
      Value(
        properties: (repeat (each ValueProperty)(
          propertySchema: (each propertyValues)
        ))
      )
    }
    static func propertyValues(from value: Value) -> PropertyValues {

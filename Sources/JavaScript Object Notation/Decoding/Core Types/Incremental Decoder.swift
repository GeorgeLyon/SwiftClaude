...
public final class IncrementalDecoder {
  ...
  fileprivate actor PrivateDecoder: SerialExecutor, TaskExecutor {
    ...
    /// `nonisolated` to allow this to be called from `IncrementalDecoder`.
    /// This is safe because `PrivateDecoder` runs on `executor` and `executor.run()` is only called from `IncrementalDecoder` (which is non-sendable).
    ...
    /// Should only be called from the owning `IncrementalDecoder`'s  isolation domain
...
final class DecodingSession<Value> {
  ...
  fileprivate init(decoder: IncrementalDecoder.PrivateDecoder) {
    self.decoder = decoder
  }
  fileprivate let decoder: IncrementalDecoder.PrivateDecoder
